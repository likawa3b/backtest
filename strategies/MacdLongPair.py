...
def initiate(upper, population):
    ...
    candidates['topfilter'] = np.random.randint(0, 200, size=population)
    candidates['botfilter'] = np.random.randint(0, 200, size=population)
    candidates['mutation'] = None
    return candidates, no_paras


class StMacdLongPair(bt.Strategy):
    params = (
        ('fastlen', 12),
        ('slowlen', 26),
        ('signallen', 9),
    ...
    def __init__(self):
        ...
        ####################################################################################################
        self.macd_pair = bt.indicators.MACD(self.dataclose,
                                       period_me1=48,
                                       period_me2=154,
                                       period_signal=14,
                                       plot=False)

        self.macdcross_pair = bt.indicators.CrossOver(
            self.macd_pair.macd, self.macd_pair.signal, plot=True)
        
        self.close_macd_pair = bt.indicators.MACD(self.dataclose,
                                             period_me1=9,
                                             period_me2=20,
                                             period_signal=3,
                                             plot=False)

        self.close_macdcross_pair = bt.indicators.CrossOver(
            self.close_macd_pair.macd, self.close_macd_pair.signal, plot=False)
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if (self.macdcross[0] == 1 and self.macd.macd[0] <= -self.params.botfilter) or (self.macdcross_pair[0] ==1 and self.macd_pair.macd[0]<=-39):
            ...
            if (self.close_macdcross[0] == -1 and self.close_macd.macd[0] >= self.params.topfilter) or (self.close_macdcross_pair[0] == -1 and self.close_macd_pair.macd[0] >= 155):
    ...
    def stop(self):
        self.log('(Entry: %d\\%d\\%d botFilter: %d)(Close: %d\\%d\\%d topFilter: %d) Cutloss: %.1f Percent: %d Commission: %.1f%% Ending Value %.2f' %
                 (self.params.fastlen, self.params.slowlen, self.params.signallen, self.params.botfilter,
                  self.params.close_fastlen, self.params.close_slowlen, self.params.close_signallen, self.params.topfilter,
